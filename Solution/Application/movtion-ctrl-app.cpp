...
// 1. 无锁极速读取意图和状态 (注意调用的是大写的 Read)
        ...
        for (int i = 0; i < 4; i++) {
            ...
            output.steerVoltage[motorIdx[i]] = steerSpdPid[motorIdx[i]].calculate(tgtSteerSpd, state.modules[motorIdx[i]].steer.vel);

            // (5) 动力轮：单环速度 PID 控制
            output.driveCurrent[motorIdx[i]] = driveSpdPid[motorIdx[i]].calculate(tgtSpeed, state.modules[motorIdx[i]].drive.vel);
        }
